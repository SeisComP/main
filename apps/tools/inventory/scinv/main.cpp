...
class InventoryManager : public Client::Application,
	...
	public:
		...
		bool send(DataModel::NotifierMessage *nmsg) {
			Client::Result r;
			if ( (r = connection()->sendMessage(nmsg)) != Client::OK ) {
				if ( r == Client::MessageTooLarge ) {
					SEISCOMP_WARNING("Message with %d notifiers is too large, will split it",
					                 nmsg->size());
					...
					SEISCOMP_ERROR("%s (%d): %s", r.toString(), r.toInt(),
					               connection()->lastErrorMessage().c_str());
				}
			}
