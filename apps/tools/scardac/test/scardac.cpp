...
#define ASSERT_EQUAL_MSG(obj1, obj2, msg) \
	ASSERT_MSG(obj1 == obj2, msg << ": " << #obj1 " == " #obj2 " has failed [" << obj1 \
	                             << " != " << obj2 << "]");

#define CHECK_EQUAL_MSG(obj1, obj2, msg) \
	if ( obj1 != obj2 ) { \
		BOOST_ERROR(msg << ": " << #obj1 " == " #obj2 " has failed [" << obj1 \
		                << " != " << obj2 << "]"); \
	}
...
void checkObjectCount(DataModel::DatabaseReaderPtr &reader, const string &ctx,
                      ...
                      size_t segments = 0) {
	CHECK_EQUAL_MSG(extents,
	                reader->getObjectCount(nullptr, DataModel::DataExtent::TypeInfo()),
	                ctx);
	CHECK_EQUAL_MSG(
	    attributeExtents,
	    reader->getObjectCount(nullptr, DataModel::DataAttributeExtent::TypeInfo()),
	    ctx);
	CHECK_EQUAL_MSG(segments,
	                reader->getObjectCount(nullptr, DataModel::DataSegment::TypeInfo()),
	                ctx);
}

DataModel::DataAvailabilityPtr checkEqual(DataModel::DatabaseReaderPtr &reader,
                                          const DataModel::DataAvailability *daExpected,
                                          const string &ctx) {
	...
	for ( size_t iExt = 0; iExt < daExpected->dataExtentCount(); ++iExt ) {
		...
		CHECK_EQUAL_MSG(extExpected->segmentOverflow(), extFound->segmentOverflow(),
		                ctx);
		...
		ASSERT_EQUAL_MSG(extExpected->dataSegmentCount(), extFound->dataSegmentCount(),
		                 ctx << ": Different number of DataSegments");

		for ( size_t iAttExt = 0; iAttExt < extExpected->dataAttributeExtentCount();
		      ++iAttExt ) {
			...
			if ( *attExtExpected != *attExtFound ) {
				...
				BOOST_CHECK_EQUAL(attExtExpected->start().iso(),
				                  attExtFound->start().iso());
				BOOST_CHECK_EQUAL(attExtExpected->end().iso(),
				                  attExtFound->end().iso());
				BOOST_CHECK_EQUAL(attExtExpected->updated().iso(),
				                  attExtFound->updated().iso());
				BOOST_CHECK_EQUAL(attExtExpected->segmentCount(),
				                  attExtFound->segmentCount());
			}
		}
		...
		for ( size_t iSeg = 0; iSeg < extExpected->dataSegmentCount(); ++iSeg ) {
			...
			if ( *segExpected != *segFound ) {
				...
				BOOST_CHECK_EQUAL(segExpected->updated().iso(),
				                  segFound->updated().iso());
...
void copy(const string &src, const string &dst, bool contentOnly = false,
	...
	try {
		...
		ASSERT_MSG(fs::exists(from), "Could not copy " << src << " to " << dst
		...
		if ( fs::is_directory(from) ) {
			...
			if ( !fs::exists(to) ) {
				createPathToDst ? fs::create_directories(to) : fs::create_directory(to);
			}
			...
			if ( createPathToDst ) {
				...
				if ( SC_FS_HAS_PARENT_PATH(to) && !fs::exists(SC_FS_PARENT_PATH(to)) ) {
...
struct F {
	...
	~F() { BOOST_TEST_MESSAGE("Fixture destroyed"); }
...
BOOST_AUTO_TEST_CASE(basic) {
	...
	BOOST_TEST_MESSAGE(ctx);
	ASSERT_MSG(fs::remove(SC_FS_PATH(mseedFile)), "Could not remove: " << mseedFile);
	reader = runApp(
	    dbURI, { appName, "--deep-scan", "--exclude", "AM.*", "--include", "GE.*" });
	checkObjectCount(reader, ctx, 1, 1, 1);

	ctx = "rescan: empty archive, stream ignored (2) ";
	BOOST_TEST_MESSAGE(ctx);
	reader = runApp(dbURI, { appName, "--deep-scan", "--include", "AM.*.SHX" });
	checkObjectCount(reader, ctx, 1, 1, 1);

	ctx = "rescan: empty archive, stream matched ";
	BOOST_TEST_MESSAGE(ctx);
	reader = runApp(dbURI, { appName, "--deep-scan", "--include", "AM.*.SHZ" });
...
BOOST_AUTO_TEST_CASE(multiday) {
	...
	auto mseedFile0 = basePath + "239"; // seg0
	auto mseedFile1 = basePath + "240"; // seg0, seg1, seg2
	...
	lastScan.setUSecs(0);
	auto future = lastScan + twoSeconds;
	...
	auto daFound = checkEqual(reader, da.get(), ctx);
	...
	*seg2b = *seg2;
	...
	seg2a->setUpdated(seg1->updated());
	seg2a->setEnd(Core::Time(2023, 8, 29, 0, 0, 5, 801'000));
	seg2b->setStart(Core::Time(2023, 8, 30, 0, 0, 2, 881'000));
	...
	auto past = lastScan + twoSeconds;
	// use past mtime to test re-read is triggered even last scan is more recent
	boost::filesystem::last_write_time(mseedFile2, past.epochSeconds());

	ext->removeDataSegment(2); // seg2a
	ext->removeDataSegment(2); // seg2b
...
BOOST_AUTO_TEST_CASE(timewindow) {
	...
	ctx = "end on microsecond before record start";
	BOOST_TEST_MESSAGE(ctx);
	reader = runApp(dbURI, { appName, "--end", "2023-08-27T22:59:56.940" });
	checkObjectCount(reader, ctx);
	...
	BOOST_TEST_MESSAGE(ctx);
	reader = runApp(
	    dbURI, { appName, "--start", "2023-08-30T01:00:05", "--modified-since", "1" });
