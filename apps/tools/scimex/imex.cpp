...
bool ImEx::init() {
	...
	if ( groupSubscriptions.empty() ) {
		if ( _mode == IMPORT ) {
			groupSubscriptions.push_back(Client::Protocol::IMPORT_GROUP);
		}
	...
	for ( auto &&item : groupSubscriptions )
		connection()->subscribe(item.c_str());
...
// >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
void ImEx::handleNetworkMessage(const Client::Packet *msg) {
...
void ImEx::handleMessage(Core::Message* message) {
	...
	if ( !_importMessageConversion.empty() ) {
		if ( _lastNetworkMessage->type == Client::Packet::Data ) {
...
Core::MessagePtr ImEx::convertMessage(Core::Message* message) {
	...
	boost::iostreams::stream_buffer<boost::iostreams::array_source> buf(
		_lastNetworkMessage->payload.c_str(), _lastNetworkMessage->payload.size()
	);

	filtered_buf.push(boost::iostreams::zlib_decompressor());
	filtered_buf.push(buf);

	std::unique_ptr<IO::Importer> importer = std::unique_ptr<IO::Importer>(IO::ImporterFactory::Create(_importMessageConversion.c_str()));
	if ( !importer.get() ) {
