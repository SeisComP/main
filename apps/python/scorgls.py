    ...
    def init(self):
        ...
        try:
            ...
            if not self._startTime.fromString(start, "%F %T"):
                print(
                    f"Wrong 'begin' given -> assuming {self._startTime}",
                    file=sys.stderr,
                )
        except RuntimeError:
            print(f"No 'begin' given -> assuming {self._startTime}", file=sys.stderr)

        try:
            end = self.commandline().optionString("end")
            if not self._endTime.fromString(end, "%F %T"):
                print(f"Wrong 'end' given -> assuming {self._endTime}", file=sys.stderr)
        except RuntimeError:
            print(f"No 'end' given -> assuming {self._endTime}", file=sys.stderr)

        try:
            self.author = self.commandline().optionString("author")
            sys.stderr.write(f"{self.author} author used for output\n")
        except RuntimeError:
            self.author = False
